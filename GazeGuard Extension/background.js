if (chrome.storage.sync.syncInit) {
    ...
    localStorage.istypeModel = chrome.storage.sync.istypeModel;
    localStorage.isBlackList = chrome.storage.sync.isBlackList;
    localStorage.isYolov5 = chrome.storage.sync.isYolov5;
...
var ul = localStorage.urlList, urlList = ul ? JSON.parse(ul) : [],
    ...
    istypeModel = localStorage.istypeModel == 1,
    isBlackList = localStorage.isBlackList == 1,
    isYolov5 = localStorage.isYolov5 =5,
    excludeForTabList = [],
    pauseForTabList = [],
    domainRegex = /^\w+:\/\/([\w\.:-]+)/;
    ...
    function (request, sender, sendResponse) {
        switch (request.r) {
            ...
            case 'setNoEye':
                ...
                break;
            case 'settypeModel':
                istypeModel = request.toggle;
                localStorage.istypeModel = istypeModel ? 1 : 0;
                break;
            ...
            case 'setYolov5':
                console.log("setYolov5");
                isYolov5 = request.toggle;
                localStorage.isYolov5 = isYolov5 ? 1 : 0;
                 break;
        }
    }
);